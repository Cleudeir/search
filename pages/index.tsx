...
export default function Home() {
  ...
  const [_data, _setData] = useState<DataTv | null>(null)
   return( 
   <div>
    ...
    data && <>
      ...
      <main className={styles.main}>
        <input type='text' value={text} onChange={(e)=>{
          ...
          setSearchMovie(data.movie.filter((item: { title: string | string[]; })=> item.title.includes(e.target.value)).slice(0,10));
          setSearchTv(data.tv.filter((item: { title: string | string[]; })=> item.title.includes(e.target.value)).slice(0,10))
        }}/>
        <div className={styles.movies}>
          <h2>Movies</h2>
        {searchMovie.length> 0 && searchMovie.map((item: { title: string  })=>(
        <Card type='movie' item={item} key={item.title}/>
        ))}
        </div>
        <div className={styles.movies}>
          <h2>Tv</h2>
        {searchTv.length> 0 && searchTv.map((item: { title: string })=>(
